...
def make_delta(base_model_path, target_model_path, delta_path, hub_repo_id):
    ...
    target = ValleyLlamaForCausalLM.from_pretrained(
    ...
    for name, param in tqdm(target.state_dict().items(), desc="Calculating delta"):
        if name not in base.state_dict():
            assert (
                name in ["model.mm_projector.weight", "model.mm_projector.bias"]
    ...
    if hub_repo_id:
        kwargs = {
            "push_to_hub": True,
            "repo_id": hub_repo_id,
            "use_auth_token": "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",
        }
    else:
        kwargs = {}
