...
class ValleyLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "use_delta_transformer") and config.use_delta_transformer:
            ...
            self.position_matrix = torch.nn.Parameter(
                torch.zeros(2048, config.hidden_size)
            )
    ...
    def temporal_tranforemr_delta_adding(self, patch_feature):  # 8, 256, 5120
        ...
        position_embedding = (
            self.position_matrix[:sequence_length, :]
            .unsqueeze(0)
            .type_as(patch_feature)
        )  # 1,8,5120
        ...
        patch_feature_delta = self.transformer_delta_encoder(patch_feature_pos)[
            :, -1, :
        ]
        ...
        if (
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                for (
                    ...
                    cur_image_features = mean_image_features.to(
                ...
                try:
                    ...
                    cur_video_input_embeds = (
                        cur_new_input_embeds.clone()
                    )  # to save the new embed
                    for (
                        video_start_token_pos
                    ) in (
                        video_start_tokens
                    ):  # this loop is for multi_image in one piece
                        ...
                        cur_video_input_embeds = torch.cat(
                            (
                                ...
                                cur_video_input_embeds[
                                    video_start_token_pos + num_frame + 1 :
                                ],
                            ),
                            dim=0,
                        )
...
class ValleyLlamaForCausalLM(LlamaForCausalLM):
    ...
    def build_inputs(self, tokenizer, messages):
        ...
        for m in messages:
            if m["role"] == "system":
                ...
                raise ValueError(
                    'Role is only suport "assistent", "human" and "system".'
                )
    ...
    @torch.no_grad()
    def completion(
        self, tokenizer, video: str, message: list, gen_kwargs: dict, device
    ):
