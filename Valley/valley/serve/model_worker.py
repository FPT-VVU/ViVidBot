...
class ModelWorker:
    ...
    def load_video(self, path):
        ...
        video = video_reader.get_batch(
            np.linspace(0, video_len - 1, 8).astype(np.int_)
        ).byte()
    ...
    def generate_video_stream(self, params):
        ...
        if videos is not None and len(videos) > 0 and self.is_multimodal:
            ...
            if len(videos) > 0:
                ...
                assert len(videos) == prompt.count(
                    DEFAULT_VIDEO_TOKEN
                ), "Number of video does not match number of <video> tokens in prompt"
                ...
                if getattr(self.model.config, "mm_use_im_start_end", False):
                    replace_token = (
                        ...
                        + DEFAULT_IM_END_TOKEN
                        + DEFAULT_VI_START_TOKEN
                        + DEFAULT_VIDEO_FRAME_TOKEN * videos.shape[0]
                        + DEFAULT_VI_END_TOKEN
                    )
        ...
        for i in range(max_new_tokens):
            if i == 0:
                out = model(
                    torch.as_tensor([input_ids]).cuda(), use_cache=True, **video_args
                )
                logits = out.logits
                past_key_values = out.past_key_values
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--worker-address", type=str, default="http://localhost:39999")
    parser.add_argument(
        "--controller-address", type=str, default="http://localhost:20000"
    )
    ...
    parser.add_argument(
        "--multi-modal",
        action="store_true",
        help="Multimodal mode is automatically detected with model name, please make sure `valley` is included in the model path.",
    )
    ...
    if args.multi_modal:
        logger.warning(
            "Multimodal mode is automatically detected with model name, please make sure `valley` is included in the model path."
        )
