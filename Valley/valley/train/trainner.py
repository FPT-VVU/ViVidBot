        ...
        with open(prediction_file_name, "w") as writer:
            for file_name in old_files:
                with open(file_name, "r") as reader:
                    for line in reader:
                        metrics.append(
                            torch.tensor(self.compute_dev_metric(json.loads(line)))
                        )
                        writer.write(f"{line}")
...
class ValleyTrainer(Seq2SeqTrainer):
    ...
    def create_optimizer(
        ...
        if self.args.lora and self.args.tune_mm_mlp_adapter:
            ...
            optimizer_grouped_parameters = [
                {
                    ...
                    "lr": (
                        float(self.args.lora_lr)
                        if self.args.lora_lr
            ...
            optimizer_cls, optimizer_kwargs = (
                ValleyTrainer.get_optimizer_cls_and_kwargs(self.args)
            )
    ...
    def prediction_step(
        ...
        response_input_id_list = [
            input_id
            for i, input_id in enumerate(input_ids_split)
            if i % 2 == 0 and i != 0
        ]
