...
class ThreadPool:
    ...
    def get_result(self, future):
        # 监控线程完成进度
        self.show_process(
            "任务完成进度", self.task_num - len(self.args_list), self.task_num
        )
...
def download(save_dir, jmId):
    ...
    response = requests.post(
        "https://www.jukinmedia.com/api/public/video/downloadVideo/" + jmId,
        headers=headers,
    )
...
def check_already(save_dir, args_list):
    already_file = list(
        map(lambda x: x.name.split(".")[0].split("_")[1], Path(save_dir).rglob("*.mp4"))
    )
    result = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_file",
        default="./jukinmedia/jukin-100k.json",
    )
