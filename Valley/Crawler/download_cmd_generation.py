...
def crosscheck_videos(video_path, ann_file):
    ...
    if "VATEX" in video_path:
        all_vids = list(
            set(["_".join(item["videoID"].split("_")[:-2]) for item in anet_v_1_0])
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video_path", required=True, help="Where are located the videos? (Full path)"
    )
    parser.add_argument(
        "--ann_file", required=True, help="Where is the annotation file?"
    )
    parser.add_argument(
        "--output_filename",
        default="./VATEX/cmd_list.txt",
        required=True,
        help="Output script location.",
    )
