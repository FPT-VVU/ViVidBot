...
class LLMCallback(TrainerCallback):
  ...
  def on_step_end(
    ...
    **kwargs,
    ...
    if args.lora_save_strategy == "steps" and state.global_step % args.save_steps == 0:
      ...
      if (
        args.lora and peft_str in str(type(model_)) and not is_deepspeed_zero3_enabled()
      ):
  ...
  def merge_files(self, prediction_file_name):
    ...
    with open(prediction_file_name, "w") as writer:
      ...
      (
        ...
        bert_score_recall,
        bert_score_f1,
      ) = metrics.numpy().tolist()
    with open(
      prediction_file_name.replace(".jsonl", "_metric.txt"), "w"
    ) as metric_writer:
  ...
  def compute_dev_metric(self, d):
    ...
    bleu = (
      sum([metric["bleu"] for metric in metric_score]) / len(metric_score)
      if len(metric_score) != 0
      else 0
    )
    rouge1 = (
      sum([metric["rouge1"] for metric in metric_score]) / len(metric_score)
      if len(metric_score) != 0
      else 0
    )
    rouge2 = (
      sum([metric["rouge2"] for metric in metric_score]) / len(metric_score)
      if len(metric_score) != 0
      else 0
    )
    rougeL = (
      sum([metric["rougeL"] for metric in metric_score]) / len(metric_score)
      if len(metric_score) != 0
      else 0
    )
    rougeLsum = (
      sum([metric["rougeLsum"] for metric in metric_score]) / len(metric_score)
      if len(metric_score) != 0
      else 0
    )
    bert_score_precision = (
      sum(bert_score["precision"]) / len(bert_score["precision"])
      if len(bert_score["precision"]) != 0
      else 0
    )
    bert_score_recall = (
      sum(bert_score["recall"]) / len(bert_score["recall"])
      if len(bert_score["recall"]) != 0
      else 0
    )
    bert_score_f1 = (
      sum(bert_score["precision"]) / len(bert_score["precision"])
...
class VividTrainer(Seq2SeqTrainer):
  ...
  def create_optimizer(
    ...
    if self.args.lora and self.args.tune_mm_mlp_adapter:
      ...
      optimizer_grouped_parameters = [
        {
          ...
          "lr": (
            float(self.args.lora_lr) if self.args.lora_lr else self.args.learning_rate
          ),
        },
        {
          "params": [
            p
            for n, p in self.model.named_parameters()
            if (n in decay_parameters and "lora" not in n and p.requires_grad)
          ],
          "weight_decay": self.args.weight_decay,
        },
        {
      ...
      optimizer_cls, optimizer_kwargs = VividTrainer.get_optimizer_cls_and_kwargs(
        self.args
      )
  ...
  def prediction_step(
    ...
    human_input_id_list = [
      input_id for i, input_id in enumerate(input_ids_split) if i % 2 == 1
    ]
    begin_ids = human_input_id_list[-1]
    human_input_id_list = human_input_id_list[:-1]
    response_input_id_list = [
      input_id for i, input_id in enumerate(input_ids_split) if i % 2 == 0 and i != 0
    ]
    ...
    for turn_index in range(turn_number):
      input_ids = torch.concat(
        [last_id, human_input_id_list[turn_index], begin_ids], dim=1
      )
      ...
      gen_kwargs["stopping_criteria"] = [
        KeywordsStoppingCriteria(["###"], self.tokenizer, inputs["input_ids"])
      ]
    ...
    self._output_generate_results(generated_tokens, gd_truth_tokens)
    kwargs = {"device": self.args.device}
    generated_tokens = torch.tensor([1]).to(**kwargs)
    loss = None
    labels = None
  ...
  def _output_generate_results(self, generated_tokens, gd_truth):
    ...
    for generate_str, gd_truth_str in zip(generated_tokens, gd_truth):
      try:
        ...
        metric_score.append(
          self.clf_metrics.compute(
            predictions=[generate_response_list[-1]],
            references=[gd_truth_list[-1]],
          )
        )
    ...
    bert_score = self.bertscore.compute(
      predictions=generate_response_list, references=gd_truth_list, lang="en"
    )
