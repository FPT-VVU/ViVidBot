...
class DataArguments:
  ...
  fashion_data_path: str = field(
    default=None, metadata={"help": "Path to the new construct training data."}
  )
  video_data_path: Optional[str] = field(
    default=None, metadata={"help": "Path to the video training data."}
  )
...
def train(args):
  ...
  if training_args.tune_mm_mlp_adapter:
    ...
    model.get_model().orig_embeds_params = [
      model.get_input_embeddings().weight.data.clone().to(device=training_args.device)
    ]
  ...
  data_module = make_video_supervised_data_module(
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--conf", type=str, default="valley/configs/experiment/valley_debug.yaml"
  )
