...
def load_video(path, image_processor):
  if os.path.isfile(path):
    ...
    if len(video_frames) > 8:
      video_frames = [
        video_frames[i]
        for i in np.linspace(0, len(video_frames) - 1, 8).astype(np.int_)
      ]
      ...
      if not conv.has_video:
        if mm_use_im_start_end:
          qs = (
            ...
            + DEFAULT_IM_END_TOKEN
            + DEFAULT_VI_START_TOKEN
            + DEFAULT_VIDEO_FRAME_TOKEN * video_length
            + DEFAULT_VI_END_TOKEN
          )
        else:
          qs = qs + "\n" + DEFAULT_IMAGE_PATCH_TOKEN * image_token_len
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    ...
    type=str,
    required=False,
    default="Describe the following video concisely.",
  )
  parser.add_argument(
    "--video_file",
    type=str,
    required=False,
    default="/mnt/bn/luoruipu-disk/LLaVa-personal/save_images/live_frame/1",
  )
