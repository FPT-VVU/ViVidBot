...
class VividLlamaModel(LlamaModel):
  ...
  def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
    ...
    if (
      ...
      and config.use_patch_importance_pooling
    ):
  ...
  def initialize_vision_modules(
    ...
    if not hasattr(self, "transformer_delta_encoder") and use_delta_transformer:
      ...
      self.position_matrix = torch.nn.Parameter(
        self.getPositionEncoding(seq_len=2048, d=self.config.hidden_size)
      )
      self.position_matrix.requires_grad = False
  ...
  def temporal_tranforemr_delta_adding(self, patch_feature):  # 8, 256, 5120
    ...
    position_embedding = (
      self.position_matrix[:sequence_length, :].unsqueeze(0).type_as(patch_feature)
    )  # 1,8,5120
    position_embedding = position_embedding.repeat(patch_number, 1, 1).to(
      patch_feature.device
    )  # 256,8,5120
  ...
  def forward(
    ...
    if (
      ...
      if type(vision_tower) is list and len(vision_tower) >= 1:
        ...
        if type(images) is list:
          ...
          for batch_id in range(len(images)):
            image_forward_outs = vision_tower(
              images[batch_id], output_hidden_states=True
            )  # 8,3,224,224
            select_hidden_state_layer = getattr(
              self.config, "mm_vision_select_layer", -1
            )
      ...
      if type(images) is list:
        image_features = [
          self.mm_projector(image_feature) for image_feature in image_features
        ]
      else:
      ...
      for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
        ...
        if self.patch_pooling_method == "mean":
          mean_image_features = torch.mean(
            cur_image_features[:, 1:, :], dim=0
          )  # 256 , 4096
        elif self.patch_pooling_method == "max":
          mean_image_features = torch.max(cur_image_features[:, 1:, :], dim=0)[
            0
          ]  # 256 , 4096
        elif self.patch_pooling_method == "temporal_importance":
          mean_image_features = self.text_importance_pooling(
        ...
        if (cur_input_ids == vision_tower.config.im_start_token).sum() != (
          ...
          raise ValueError(
            "The number of im_start_token and im_end_token should be the same"
          )
        ...
        for (
        ...
        ) in image_start_tokens:  # this loop is for multi_image in one piece
          ...
          cur_new_input_embeds = torch.cat(
            (
              cur_new_input_embeds[: image_start_token_pos + 1],
              cur_image_features,
              cur_new_input_embeds[image_start_token_pos + num_patches + 1 :],
            ),
            dim=0,
          )
          multi_iamge_index += 1
        ...
        try:
          if (cur_input_ids == vision_tower.config.vi_start_token).sum() != (
            cur_input_ids == vision_tower.config.vi_end_token
          ).sum():
            raise ValueError(
              "The number of vi_start_token and vi_end_token should be the same"
            )
          video_start_tokens = torch.where(
            cur_input_ids == vision_tower.config.vi_start_token
          )[0]
          num_frame = frame_image_features.shape[0]
          assert (
            cur_input_ids == vision_tower.config.vi_frame_token
          ).sum() == num_frame
          cur_video_input_embeds = cur_new_input_embeds.clone()  # to save the new embed
          for (
            video_start_token_pos
          ) in video_start_tokens:  # this loop is for multi_image in one piece
            frame_image_features = frame_image_features.to(
              device=cur_input_embeds.device
            )
            if (
              cur_input_ids[video_start_token_pos + num_frame + 1]
              != vision_tower.config.vi_end_token
            ):
              raise ValueError("Seems that the image is cut.")
            cur_video_input_embeds = torch.cat(
              (
                cur_video_input_embeds[: video_start_token_pos + 1],
                frame_image_features,
                cur_video_input_embeds[video_start_token_pos + num_frame + 1 :],
              ),
              dim=0,
            )
    ...
    return super(VividLlamaModel, self).forward(
      input_ids=None,
...
class VividLlamaForCausalLM(LlamaForCausalLM):
  config_class = VividConfig
  ...
  def initialize_vision_tokenizer(self, tokenizer):
    ...
    tokenizer.add_tokens(
      [DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_VIDEO_FRAME_TOKEN], special_tokens=True
    )
...
AutoConfig.register("vivid_llama", VividConfig)
AutoModelForCausalLM.register(VividConfig, VividLlamaForCausalLM)
