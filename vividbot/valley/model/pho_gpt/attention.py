  ...
  if should_repeat_kv_for_gqa:
    if kv_n_heads == 1:
      ...
      key_unpad = repeat_kv_for_gqa(
        key_unpad.view(1, key_unpad.size(0), kv_n_heads, -1), n_heads // kv_n_heads
      ).view(key_unpad.size(0), n_heads, -1)
      value_unpad = repeat_kv_for_gqa(
        value_unpad.view(1, value_unpad.size(0), kv_n_heads, -1), n_heads // kv_n_heads
...
class GroupedQueryAttention(nn.Module):
  ...
  def __init__(
    ...
    self.Wqkv = FC_CLASS_REGISTRY[fc_type](
      self.d_model, self.d_model + 2 * self.kv_n_heads * self.head_dim, **fc_kwargs
    )
    fuse_splits = [
  ...
  def forward(
    ...
    (query, key, value) = qkv.split(
      [self.d_model, self.kv_n_heads * self.head_dim, self.kv_n_heads * self.head_dim],
      dim=2,
    )
