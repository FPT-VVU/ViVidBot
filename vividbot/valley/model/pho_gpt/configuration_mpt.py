...
class MptConfig(PretrainedConfig):
  ...
  def __init__(
    ...
    **kwargs: Any,
    ...
    if self.attn_config.get("alibi", False) or self.attn_config.get("rope", False):
      ...
      warnings.warn("alibi or rope is turned on, setting `learned_pos_emb` to `False.`")
  ...
  def _validate_config(self) -> None:
    ...
    if not (
      self.learned_pos_emb or self.attn_config["alibi"] or self.attn_config["rope"]
    ):
      warnings.warn(
        "Positional information not being provided to the model using either learned_pos_emb or alibi or rope."
      )
    ...
    if self.ffn_config["ffn_type"] == "mptgeglu":
      ...
      if "ffn_act_fn" in self.ffn_config.keys():
        raise ValueError(
          "Transformer Engine block does not support custom activation functions."
        )
