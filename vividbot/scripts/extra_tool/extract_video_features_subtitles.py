...
def parse_args():
  parser = argparse.ArgumentParser(
    description="Extract CLIP feature and subtitles for a video"
  )

  parser.add_argument(
    "--video_file", required=True, help="Path to read the videos from."
  )
  parser.add_argument(
...
def load_subtitles(video_file):
  ...
  timestamps = [
    segment for segment in timestamps if segment["avg_logprob"] >= MIN_CONFIDENCE
  ]
  subtitles = []
...
def main():
  ...
  feat_path = (
    os.path.join(feat_dir, os.path.basename(video_file))
    .replace(".mp4", ".pkl")
    .replace(".mkv", ".pkl")
  )
  print(feat_path)
  ...
  video_tensor = image_processor.preprocess(video, return_tensors="pt")[
    "pixel_values"
  ].half()
