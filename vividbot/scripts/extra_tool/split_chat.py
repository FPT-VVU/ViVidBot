...
def split_chat(
  input_path, output_path, max_round, media_token="<image>", img_token="<image>"
):
  ...
  for _i, _data in enumerate(raw_data):
    ...
    if "image" in _data or "video" in _data:
      for _idx, _conv in enumerate(_data["conversations"]):
        ...
        if (
          len(conv_temp["conversations"]) == 2 * max_round
          or _idx == len(_data["conversations"]) - 1
        ):
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--input", type=str, default="data/processed_data/processed_chat.json"
  )
  parser.add_argument(
    "--output", type=str, default="data/processed_data/processed_chat.json"
  )
