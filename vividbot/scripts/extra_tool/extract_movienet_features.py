...
from vividbot.llamavid.model.multimodal_encoder.eva_vit import EVAVisionTowerLavis
import tarfile
import io
import re
import datetime
...
def parse_args():
  ...
  parser.add_argument(
    "--video_dir", required=True, help="Path to read the videos from."
  )
  parser.add_argument(
    "--files_dir", required=True, help="Path to read the shot detection result from."
  )
  parser.add_argument(
    "--feat_dir", required=True, help="The output dir to save the features in."
  )
  parser.add_argument(
    "--vision_tower",
    default="./model_zoo/LAVIS/eva_vit_g.pth",
    help="Vision backbone to process the video.",
  )
  parser.add_argument(
    "--image_processor",
    defalut="./llamavid/processor/clip-patch14-224",
    help="Image processor to pre-process the video.",
  )
...
def load_subtitles(file_path):
  def check(subtitle):
    if subtitle.get("text", None) is None:
      return False
    if subtitle.get("start_time", None) is None:
      return False
    if subtitle.get("end_time", None) is None:
      return False
    return True
...
def main():
  ...
  vision_tower = EVAVisionTowerLavis(
    args.vision_tower, args.image_processor, args=None
  ).cuda()
  vision_tower.eval()
  ...
  for video_path in tqdm(video_files):
    ...
    if not os.path.exists(script_path):
      continue
    if args.compute_feat and os.path.exists(feat_path):
      continue
    ...
    video_tensor = image_processor.preprocess(image_data, return_tensors="pt")[
      "pixel_values"
    ].half()
