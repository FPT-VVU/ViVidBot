...
class Executor(BaseProcessor):
    def __init__(self, file_path: str, 
        ...
        self.dataset = self.load_dataset() if self.num_file_temp == 0 and self.num_file_result == 0 else None
        self.uploader = Uploader()

    
    def load_dataset(self):
        if not os.path.exists(self.file_path) or self.file_path.split("/")[-1].split(".")[-1] != "json":
