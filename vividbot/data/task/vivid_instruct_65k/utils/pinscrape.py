...
class PinterestImageScraper:
  ...
  def get_source(self, url: str, proxies: dict) -> None:
    ...
    self.json_data_list.append(json.loads(json_data[0].string)) if len(
      json_data
    ) else self.json_data_list.append({})
  ...
  def saving_op(self, var):
    ...
    for img in url_list:
      ...
      if self.dhash(image) not in self.unique_img:
  ...
  def start_scraping(max_images, key=None, proxies: dict = {}):
    ...
    searched_urls, links = PinterestImageScraper.get_pinterest_links(
  ...
  def scrape(
    ...
    extracted_urls, keyword, search_engine_status_code, links = (
      PinterestImageScraper.start_scraping(max_images, key, proxies)
    )
    self.unique_img = []
    self.json_data_list = []
