...
class LLaMAVIDMetaModel:
  ...
  def initialize_attention_modules(self, model_args, for_eval=False):
    ...
    self.vlm_att_tokenlizer, self.vlm_att_encoder, self.vlm_att_query = self.init_bert(
      att_feat_size, truncation_side="left"
    )
    self.vlm_att_projector = torch.nn.Linear(
      self.vlm_att_encoder.config.hidden_size, self.config.mm_hidden_size
    )
    self.vlm_att_key_projector = torch.nn.Linear(
      self.config.mm_hidden_size, self.config.mm_hidden_size
    )
    self.vlm_att_val_projector = torch.nn.Linear(
      self.config.mm_hidden_size, self.config.hidden_size
    )

    if "raw" in self.config.bert_type:
      self.vlm_att_bert_proj = torch.nn.Linear(
        att_feat_size, self.vlm_att_encoder.config.hidden_size
      )
    elif (
      "pretrain" in self.config.bert_type
      and self.config.mm_hidden_size != att_feat_size
    ):
      self.vlm_att_bert_proj = torch.nn.Linear(
        self.config.mm_hidden_size, att_feat_size
      )
    else:
    ...
    if pretrain_qformer is not None:
      ...
      bert_weight = {
        _key: qformer_weight[_key] for _key in qformer_weight if "bert" in _key
      }
    ...
    if (
      "bert.embeddings.position_ids" not in bert_dict
      and "raw_bert" not in self.config.bert_type
    ):
      bert_dict["bert.embeddings.position_ids"] = (
        self.vlm_att_encoder.bert.embeddings.position_ids
      )
    print("Loading pretrained weights...")
    self.vlm_att_encoder.load_state_dict(bert_dict)
    self.vlm_att_projector.load_state_dict(
      get_w(att_projector_weights, "vlm_att_projector")
    )
    self.vlm_att_key_projector.load_state_dict(
      get_w(att_projector_weights, "vlm_att_key_projector")
    )
    self.vlm_att_val_projector.load_state_dict(
      get_w(att_projector_weights, "vlm_att_val_projector")
    )
    ...
    if self.vlm_att_bert_proj is not None:
      ...
      self.vlm_att_bert_proj.load_state_dict(
        get_w(att_projector_weights, "vlm_att_bert_proj")
      )

    if for_eval:
      weight_type = torch.float16
      device_type = self.mm_projector[0].weight.device
      self.vlm_att_encoder = self.vlm_att_encoder.to(
        device=device_type, dtype=weight_type
      )
      self.vlm_att_projector = self.vlm_att_projector.to(
        device=device_type, dtype=weight_type
      )
      self.vlm_att_key_projector = self.vlm_att_key_projector.to(
        device=device_type, dtype=weight_type
      )
      self.vlm_att_val_projector = self.vlm_att_val_projector.to(
        device=device_type, dtype=weight_type
      )

      if "qformer" in self.config.bert_type:
        self.vlm_att_query.data = self.vlm_att_query.data.to(
          device=device_type, dtype=weight_type
        )
        if "pretrain" in self.config.bert_type:
      ...
      if self.vlm_att_bert_proj is not None:
        self.vlm_att_bert_proj = self.vlm_att_bert_proj.to(
          device=device_type, dtype=weight_type
        )
...
class LLaMAVIDMetaForCausalLM(ABC):
  ...
  def encode_images(self, images, prompts=None, image_counts=None, long_video=False):
    ...
    image_features = self.vlm_attention(
      image_features, prompts=prompts, image_counts=image_counts, long_video=long_video
    )
  ...
  def vlm_attention(
    ...
    if image_counts is None:
      assert (
        len(image_features) == len(prompts)
      ), (
    ...
    for _idx in range(len(prompts)):
      assert isinstance(
        prompts[_idx], list
      ), f"Prompt should be a list, but got {type(prompts[_idx])}"
      input_token = (
        self.get_model()
        .vlm_att_tokenlizer(
          prompts[_idx],
      ...
      if image_counts is None:
        ...
        img_feat_prompt = (
          img_feat_prompt[None].expand(len(prompts[_idx]), -1, -1, -1).flatten(0, 1)
        )
        img_att_prompt = image_atts[total_count : total_count + image_counts[_idx]]
        img_att_prompt = (
          img_att_prompt[None].expand(len(prompts[_idx]), -1, -1).flatten(0, 1)
        )
        input_ids = input_ids[:, None].expand(-1, image_counts[_idx], -1).flatten(0, 1)
        attention_masks = (
          attention_masks[:, None].expand(-1, image_counts[_idx], -1).flatten(0, 1)
        )
        total_count += image_counts[_idx]

      if (
        "pretrain" in self.config.bert_type
        and self.get_model().vlm_att_bert_proj is not None
      ):
      ...
      if "qformer" in self.config.bert_type:
        ...
        query_atts = torch.cat(
          [
            torch.ones(query_tokens.size()[:-1], dtype=torch.long).to(bert_feat.device),
            attention_masks,
          ],
          dim=1,
        )
      ...
      final_token = self.token_generation(
        text_q, img_feat_prompt, long_video=long_video
      )

      if image_counts is not None:
        # shape: [prompt_num, frame_num*image_shape, feat_dim]
        final_token = final_token.reshape(
          len(prompts[_idx]), image_counts[_idx], *final_token.shape[-2:]
        )
  ...
  def prepare_inputs_labels_for_multimodal(
    ...
    if type(images) is list or images.ndim == 5:
      ...
      if not long_video:
        images = [
          image if len(image.shape) == 4 else image.unsqueeze(0) for image in images
        ]
      ...
      if not long_video:
        ...
        if (
          ...
          cur_new_input_embeds[text_token_indices] = (
            self.get_model().embed_tokens(cur_input_ids_text).to(device=data_device)
          )
        else:
