...
from vividbot.llamavid.model import *
from vividbot.llamavid.train.llava_trainer import LLaVATrainer
...
class DataArguments:
  ...
  video_folder: Optional[List[str]] = field(default=None)
...
class LazySupervisedDataset(Dataset):
  ...
  def __init__(
    ...
    # =========================
    # ADAPTED VERSION TO LOAD MULTIPLE DATASETS
    # =========================
    ...
    while attempt < max_attempt:
      try:
        ...
        if "image" in sources[0]:
          ...
          # =========================
          # ADAPTED VERSION TO LOAD FROM HUGGINGFACE
          # =========================

          image_file = self.list_data_dict[i]["image"]
          image_folder = self.data_args.image_folder  # image_folder is now hf repo id
          processor = self.data_args.image_processor

          image = None
          for repo_id in image_folder:
            try:
              image = load_image_hf(
                repo_path=repo_id, hf_image_path=image_file
              ).convert("RGB")
              break
            except Exception as e:
              print(f"Error in loading {i} from {repo_id}: {e}, retrying...")
              pass

          if image is None:
            raise Exception(f"Error in loading {i} from {image_folder}")
          ...
          if video is None:
            raise Exception(f"Error in loading {i} from {video_folder}")
