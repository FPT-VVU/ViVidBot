...
from vividbot.llamavid.conversation import (
  default_conversation,
  conv_templates,
  SeparatorStyle,
)
from vividbot.llamavid.constants import LOGDIR
...
def build_demo(embed_mode):
  ...
  with gr.Blocks(title="LLaMA-VID", theme=gr.themes.Default(), css=block_css) as demo:
      ...
      with gr.Column(scale=3):
        ...
        with gr.Accordion("Movie Part", open=True) as parameter_row:
          movie_part = gr.Radio(
            ...
            value=0,
            interactive=True,
            label="Part selection (plot introduced below)",
          )
    ...
    gr.Examples(
      examples=[
        [
          f"{cur_dir}/examples/Titanic.jpg",
          description_titanic,
          "Write a brief summary of this movie part.",
        ],
        [
          f"{cur_dir}/examples/Avatar.png",
          description_avatar,
          "How does human work on Pandora?",
        ],
        [
          f"{cur_dir}/examples/Avengers.jpg",
          description_avengers,
          "Who is the main antagonist in this movie, and what is his goal?",
        ],
        [
          f"{cur_dir}/examples/Interstellar.jpg",
          description_intersteller,
          "What happens in this video?",
        ],
        [
          f"{cur_dir}/examples/Forrest_Gump.jpg",
          description_gump,
          "What did Forrest Gump do in this movie part?",
        ],
      ],
    ...
    regenerate_btn.click(
      ...
      http_bot,
      [state, model_selector, movie_part, temperature, top_p, max_output_tokens],
    ...
    if args.model_list_mode == "once":
      demo.load(
        load_demo, [url_params], [state, model_selector], _js=get_window_url_params
      )
