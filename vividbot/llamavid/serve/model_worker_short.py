...
from vividbot.llamavid.constants import WORKER_HEART_BEAT_INTERVAL
from vividbot.llamavid.constants import IMAGE_TOKEN_INDEX
from vividbot.llamavid.model.builder import load_pretrained_model
from llava.mm_utils import tokenizer_image_token, KeywordsStoppingCriteria
from llava.utils import build_logger, server_error_msg, pretty_print_semaphore
from transformers import TextIteratorStreamer
from threading import Thread

from vividbot.llamavid.train.llama_flash_attn_monkey_patch import (
...
class ModelWorker:
  def __init__(
    ...
    self.is_multimodal = ("llava" in self.model_name.lower()) or (
      "vid" in self.model_name.lower()
    )
    ...
    if len(images) > 0 and len(videos) == 0:
      yield (
        json.dumps(
          {
            "text": ori_prompt
            + "Please switch to 'llama-vid-vicuna-7b-long' to chat with movie. After switch, you can clear the conversaton then retry.",
            "error_code": 0,
          }
        ).encode()
        + b"\0"
    ...
    if len(frame_idx) > 180:
      ...
      yield (
        json.dumps(
          {
            "text": ori_prompt
            + "We are sorry that, currently the maximum length of customized short videos is limited to 3 minutes.",
            "error_code": 0,
          }
        ).encode()
        + b"\0"
